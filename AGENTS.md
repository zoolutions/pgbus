...
- `create`, `edit` and `comment` all take `--attach` (all three landed in gh 2.99). Attach at create time when
  ...
  against `rake dummy:server`.
- Capture with the tool already in hand: `agent-browser screenshot <file>` or the Playwright MCP
  `browser_take_screenshot`. Save under the scratchpad, never in the repo.
